...
namespace
{
  ...
  template <int dim>
  struct AssembleTriangulationRunner
  {
  ...
  {
    constexpr int dim = 3;
    AssembleTriangulationRunner<dim> runner{.transform = Transformations<dim>::linear_transform,
  ...
  {
    constexpr int dim = 3;
    AssembleTriangulationRunner<dim> runner{.transform = Transformations<dim>::quadratic_transform,
        .function = Transformations<dim>::linear_function,
        .fill_function = Transformations<dim>::fill_hex_27};
    const double error = runner.run(false);
