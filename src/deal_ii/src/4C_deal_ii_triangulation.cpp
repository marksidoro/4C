...
namespace DealiiWrappers
{
      ...
      dealii::Triangulation<dim, spacedim>& tria, const Core::FE::Discretization& discretization)
  {
    ...
    std::tie(context.finite_elements, finite_element_names) =
    ...
    // Note: we can only create the Context that maps between the two discretizations when using pfT
    {
                  ...
                  &tria))
      {
        ...
        context.cell_index_to_element_lid.resize(tria.n_active_cells());
        context.active_fe_indices.resize(tria.n_active_cells());
        ...
        for (const auto& cell : tria.active_cell_iterators())
        {
          ...
          context.cell_index_to_element_lid[cell->active_cell_index()] = local_index;
          ...
          context.active_fe_indices[cell->active_cell_index()] =
      ...
      else
      {
        ...
        // - the cell_index_to_element_lid (-1)
        // - the active_fe_indices (dealii::numbers::invalid_unsigned_int)
        for (const auto& cell : tria.active_cell_iterators())
        {
          ...
          if (found == my_element_centers.end())
          {
            ...
            context.cell_index_to_element_lid[cell->active_cell_index()] = -1;
            context.active_fe_indices[cell->active_cell_index()] =
                dealii::numbers::invalid_unsigned_int;
            continue;
          }
