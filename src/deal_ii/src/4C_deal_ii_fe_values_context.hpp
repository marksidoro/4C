...
namespace DealiiWrappers
{
  namespace Internal
  {
    ...
    class FEValuesContextImplementation
    {
     ...
     public:
      ...
      /**
       * On the local cell/element on which this object was last reinitialized, return the
       * transformation from the deal.II shape index to the deal.II shape index.
       * I.e. for the j-th shape function in the deal.II ordering we can access its
      ...
      /**
       * Access to the underlying context object that was used to creat this object.
       */
  ...
  /**
   * \brief A wrapper/context class arond the deal.II FEValues class to evaluate finite elements.
   *
   ...
   * reinit() methods. In that step a geomtric entity (cell, element, face, etc.) is passed
   * and the FEValues object will internally precompute all the necessary data need for the
   ...
   * The design of this class is focused on on allowing the user to assemble matrices/vectors that
      ...
      const typename dealii::Triangulation<dim, spacedim>::cell_iterator& cell)
  {
    ...
    ConversionTools::FourCToDeal::reindex_shape_functions<dim, spacedim>(
        cell_data_.element->shape(), n_components, cell_data_.four_c_shape_indices);
  }
  template <int dim, int spacedim>
