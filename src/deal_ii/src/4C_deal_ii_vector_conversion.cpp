...
template <int dim, int spacedim>
Core::LinAlg::Map DealiiWrappers::create_four_c_to_dealii_map(
    const dealii::DoFHandler<dim, spacedim>& dof_handler,
    const Core::FE::Discretization& discretization, const Epetra_Map& dealii_to_four_c_map)
{
  Core::LinAlg::Vector<int> dealii_dofs(dealii_to_four_c_map);

  const auto& local_dealii_dofs = dof_handler.locally_owned_dofs();
  FOUR_C_ASSERT(
      local_dealii_dofs.n_elements() == static_cast<std::size_t>(dealii_dofs.local_length()),
      "Internal error.");
  std::copy(local_dealii_dofs.begin(), local_dealii_dofs.end(), dealii_dofs.get_values());

  Core::LinAlg::Vector<int> dealii_dofs_four_c_layout(*discretization.dof_row_map());
  Epetra_Import four_c_import(discretization.dof_row_map()->get_epetra_map(), dealii_to_four_c_map);
  dealii_dofs_four_c_layout.import(dealii_dofs, four_c_import, Insert);

  return Core::LinAlg::Map(discretization.num_global_nodes(), discretization.num_my_row_nodes(),
      dealii_dofs_four_c_layout.get_values(), 0, dealii_to_four_c_map.Comm());
}
...
{
  FOUR_C_ASSERT_ALWAYS(four_c_vector.get_map().PointSameAs(dealii_to_four_c_importer_.TargetMap()),
  ...
  FOUR_C_ASSERT(n_local_elements == dealii_to_four_c_map_.NumMyElements(), "Internal error.");
...
template Core::LinAlg::Map DealiiWrappers::create_four_c_to_dealii_map(
