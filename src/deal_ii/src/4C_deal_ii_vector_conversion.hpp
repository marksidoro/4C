...
namespace DealiiWrappers
{
  ...
  /**
   * @brief Create a Map object that maps from 4C dofs to deal.II gids.
   *
   * This function is the inverse of create_dealii_to_four_c_map().
   */
  template <int dim, int spacedim = dim>
  Core::LinAlg::Map create_four_c_to_dealii_map(
      const dealii::DoFHandler<dim, spacedim>& dof_handler,
